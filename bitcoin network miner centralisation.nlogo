...
breed [ miners a-miner ] ;;setting up with the potential to create miners with different strategies


;;Properties of bitcoin miner
miners-own [
...
to setup
  ...
  set price-change 0
  set year-counter 0
  set centralisation 100 / number-of-miners ;;calculate market share per miner
  set dead-miner-counter 0
  ask n-of number-of-miners patches [sprout-miners 1]
  ask miners [
    set shape "computer server"
    set size 1
    set hashpower-count 100 / number-of-miners
    set current-fiat-account-balance number-of-miners * cost-to-mine
...
to go
  ...
  ask miners [
    calculate-market-dominance ;;miner calculates their market dominance in order to determine their next action
  ...
  if not any? miners [user-message "All miners have ceased operations as mining is no longer sustainable from a cost perspective" stop]
  ...
  set centralisation 100 / (count miners) ;;calculate revised market share per miner
  tick
end

;;mining procedure
to mine-bitcoin
  if current-fiat-account-balance >= cost-to-mine [
    set current-fiat-account-balance current-fiat-account-balance - cost-to-mine ;;reduce the fiat balance by the cost to mine
  ]
end
...
to check-if-dead
  if current-fiat-account-balance < cost-to-mine [
    ...
    set cost-to-mine cost-to-mine * (count miners / (count miners + dead-miner-counter)) ;;cost to mine reduces by the % drop in active miners
  ]
end
...
to calibrate-current-era-reward
  if year-counter mod 48 = 0 [ ;;halvening event occurs every 4 years within the bitcoin network. Therefore the value after mod determines the period of time every tick represents
    set current-era-reward current-era-reward / 2
...
to calibrate-new-cost-to-mine
  if cost-to-mine > current-market-price * current-era-reward [
   set cost-to-mine current-market-price * current-era-reward * 0.90
  ]
end

;;procedure for calculating market dominance
to calculate-market-dominance
  set hashpower-count 100 / (count miners)
end
...
true
false
"" "set-plot-y-range 0 (number-of-miners)\nset-plot-x-range 0 (max [ total-reward ] of miners + 1)"
PENS
"default" 1.0 1 -16777216 true "" "histogram [ total-reward ] of miners"
...
"" "set-plot-y-range 0 (number-of-miners)\n"
PENS
"default" 1.0 0 -16777216 true "" "plot count miners"
